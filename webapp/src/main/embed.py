...
async def insert_embeddings(batch, embedding_url, session):
    ...
    try:
        async with session.post(embedding_url, json={"text": texts}) as response:
            ...
            values = [{"id":_id, "embedding": emb} for _id, emb in zip(ids, embeddings)]
            DBConnector.update_embedding(DBConnector.get_engine(), DBConnector.TABLE, values)
...
class FetchBatchesFunction:
    ...
    def __call__(self, batch_size):
        ...
        fetched_data = DBConnector.fetch_data_keyset(
                                ...
                                columns=[DBCOLUMNS.rowid, DBCOLUMNS.date, DBCOLUMNS.title, DBCOLUMNS.content, DBCOLUMNS.tag],
                                limit=batch_size,
                                filters={DBCOLUMNS.embedding: [("isnull", None)]}
                            )
        
        self._last_seen = {DBCOLUMNS.rowid: fetched_data[-1][0],
...
if __name__ == "__main__":
    total_rows = DBConnector.get_total_count(DBConnector.get_engine(), 
                                             DBConnector.TABLE, 
                                             filters={DBCOLUMNS.embedding: [("isnull", None)]})
    batch_size = 128
    ...
    asyncio.run(main_loop(fetch_batches_function, embedding_url, batch_size, total_rows))