...
class TestArticlesEndpoints:
    ...
    async def test_get_article_count_with_filter(
        ...
        response = await client.get(
            "/api/v1/articles/count", params={"archives": "lemonde"}
        )
        assert response.status_code == 200
        data = response.json()

        expected_count = sum(1 for a in populated_db if a["archive"] == "lemonde")
        assert data["count"] == expected_count

    @pytest.mark.asyncio
    async def test_get_date_range(self, client: AsyncClient, populated_db: List[dict]):
    ...
    @pytest.mark.asyncio
    async def test_get_tags(self, client: AsyncClient, populated_db: List[dict]):
        """Test getting tags."""
    ...
    async def test_get_group_by(self, client: AsyncClient, populated_db: List[dict]):
